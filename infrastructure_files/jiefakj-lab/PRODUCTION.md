...
- 仅改 **management** 服务端,客户端/dashboard/signal/relay 用官方镜像。本 fork 三个补丁:
  ...
  - **P2** `NB_BLOCKED_USER_MESSAGE` 可配置被拦文案(`e931f089`)——未授权用户看到申请入口。
    覆盖全部四条面向用户的拒绝路径,实现收敛在 `management/server/blockeduser`:
    peer 注册(`peer.go`)、反向代理 SSO 拒绝页(`http/handlers/proxy/auth.go`,
    上游 v0.77.1 新增)、仪表板首屏(`user.go` GetCurrentUserInfo)、
    REST 权限门(`permissions/manager.go`)。仅覆盖账号状态类拒绝,
    签名/查询失败等仍返回上游通用文案,不被改写。
